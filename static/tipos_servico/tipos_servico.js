document.addEventListener('DOMContentLoaded', function () {
    ...
    function openModal(html, mode) {
        ...
        if (mode === 'view' && modal) {
            ...
            } catch (e) { console.error('tipos_servico.openModal view-mode error', e); }
        }
    }
    ...
    async function submitForm(ev) {
        ...
        if (res.ok) {
            ...
            if (json.ok) {
                const tbody = document.querySelector('#tipos-servico-table tbody');
                const temp = document.createElement('tbody');
                temp.innerHTML = json.row_html;
                const newRow = temp.firstElementChild;
                const existing = document.getElementById('tipo-servico-row-' + json.id);
    ...
    observer.observe(document.querySelector('#tipos-servico-table tbody'), { childList: true, subtree: true });
});
