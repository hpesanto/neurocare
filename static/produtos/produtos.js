document.addEventListener('DOMContentLoaded', function () {
    ...
    function openModal(html, mode) {
        ...
        // if view-only mode, disable inputs and hide submit buttons
        if (mode === 'view' && modal) {
            try {
            ...
            } catch (e) { console.error('produtos.openModal view-mode error', e); }
        }
    }
    ...
    async function submitForm(ev) {
        ...
        if (res.ok) {
            ...
            if (json.ok) {
                ...
                const existing = document.getElementById('produto-row-' + json.id);
    ...
    observer.observe(document.querySelector('#produtos-table tbody'), { childList: true, subtree: true });
});
