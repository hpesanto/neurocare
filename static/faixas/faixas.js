document.addEventListener('DOMContentLoaded', function () {
    ...
    function openModal(html, mode) {
        ...
        if (mode === 'view' && modal) {
            ...
            } catch (e) { console.error('faixas.openModal view-mode error', e); }
        }
    }
    ...
    async function submitForm(ev) {
        ...
        if (res.ok) {
            ...
            if (json.ok) {
                ...
                const tbody = document.querySelector('#faixas-table tbody');
                const temp = document.createElement('tbody');
                temp.innerHTML = json.row_html;
                const newRow = temp.firstElementChild;
                const existing = document.getElementById('faixa-row-' + json.id);
                if (existing) existing.replaceWith(newRow);
                else tbody.appendChild(newRow);
                modalContainer.innerHTML = '';
            } else {
                // form errors returned as HTML (status 400)
    ...
    // Re-attach handlers when rows are appended/replaced
    const observer = new MutationObserver(() => attachEditHandlers());
    observer.observe(document.querySelector('#faixas-table tbody'), { childList: true, subtree: true });
});
